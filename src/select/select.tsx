...
import { OptionProps as SelectOptionProps } from './interface';
...
function Select(baseProps: SelectProps, ref) {
  const { getPrefixCls, renderEmpty, rtl } = useContext(ConfigContext);
  const props = useMergeProps<SelectProps>(baseProps, defaultProps, {});
  const {
    children,
    renderFormat,
  ...
  const getValueAndOptionForCallback = (
    ...
    if (labelInValue && !isEmpty) {
      const getOptionLabel = (optionValue: OptionProps['value'], optionInfo: OptionInfo) => {
        ...
        // https://github.com/eyas-design/eyas-design/issues/442
